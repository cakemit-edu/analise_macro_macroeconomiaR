...
df.pib <- 
  ...
  mutate(
    id_trimestre = lubridate::yq(trimestre_codigo) |> yearquarter(),
    .before=everything()
  ) |> 
  ...
  mutate(
    variavel = case_match(
      ...
      "Valores encadeados a preços de 1995" ~ "valores_encadeados95",
  ...
  mutate(
    # pib real com valores encadeados a preços do trimestre mais recente
    pib_real = (max(valores_encadeados95) / valores_encadeados95) * pib_nominal,
    
    # Variação trimestre contra trimestre imediatamente anterior (t contra t-1)
    var_trimestral = (num_indice_sa / lag(x = num_indice_sa, n = 1) - 1) * 100,
    
    # Variação trimestre contra mesmo trimestre do ano anterior (t contra t-4)
    var_interanual = (num_indice / lag(x = num_indice, n = 4) - 1) * 100,
...
df0 <- 
  inner_join(
    df.pib |> 
      select(id_trimestre, "PIB (var. % anual)" = var_anual),
...
df.pib |>  
  select(id_trimestre, var_anual) |> 
  filter(!is.na(var_anual)) |> 
  ggplot(aes(x=as_date(id_trimestre), y=var_anual)) +
